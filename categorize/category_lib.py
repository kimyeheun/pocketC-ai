...
# NOTE: 전처리에 사용되는 데이터 정리
@dataclass
class CategoryHit:
    sub_name: str  # 세부 항목
    source: str    # 어디서 걸렸는지 확인
...
REGEX_RULES: List[Tuple[re.Pattern, str]] = [
    ...
    # 유류비 (Gasoline)
    (re.compile(r"SOIL|GS칼텍스|SK에너지|현대오일뱅크|오일|에너지"), "유류비"),
    # 통신/인터넷 (Communication/Internet)
    (re.compile(r"SKT|KT|LGU\+|통신"), "휴대폰 요금"),
    # OTT (Streaming Services)
    (re.compile(r"넷플릭스|GPT|디즈니|티빙|왓챠|유튜브|웨이브|구글플레이"), "OTT/구독서비스"),
    # 쇼핑 (Shopping)
